-- ИСПРАВЛЕННАЯ ВЕРСИЯ: ТЫ НЕ ЛЕТИШЬ
...
-- ФУНКЦИЯ ОТКИДЫВАНИЯ (ТОЛЬКО ДЛЯ ЦЕЛИ)
local function LaunchPlayer(targetPlayer)
    -- Явная проверка: если это локальный игрок — выходим
    ...
    -- Проверка, что мы не применяем импульс к себе
    if targetPlayer ~= LocalPlayer then
        -- Даём импульс ТОЛЬКО цели
...
button.MouseButton1Click:Connect(function()
    ...
    if isActive then
        ...
        print("[NETERRROR] Активирован! Откидывает только других игроков.")
    else
        button.Text = "OFF"
        button.TextColor3 = Color3.fromRGB(255, 0, 0)
        button.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
...
print("[NETERRROR] Скрипт загружен! Ты НЕ откидываешься.")
